...
def detect(image_bytes, url, fallback_url=None, conf_thres=0.25, iou_thres=0.45, retries=10, delay=0):
    response = requests.post(url, data={'conf_thres':conf_thres, 'iou_thres':iou_thres}, files={'image':image_bytes})
...
def plot_box(image, top_left_point, bottom_right_point, width, height, label, color=(56, 56, 255), padding=6, font_scale=0.375, alpha=0.15):
    if alpha > 1:
        alpha = 1
